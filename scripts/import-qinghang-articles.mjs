...
async function main() {
  ...
  const missingLocalImages = imported
    .filter((article) => !LOCAL_ASSET_PATTERN.test(article.coverImage?.url || ''))
...
async function resolveCoverImage({ token, existing, sourceImages, slug, title }) {
  if (!FORCE_IMAGE_MIRROR && LOCAL_ASSET_PATTERN.test(existing?.coverImageUrl || '')) return existing.coverImageUrl
  if (!FORCE_IMAGE_MIRROR && LOCAL_ASSET_PATTERN.test(existing?.coverImage?.url || '')) return existing.coverImage.url
