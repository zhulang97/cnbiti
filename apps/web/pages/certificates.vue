<template>
  <div class="min-h-screen bg-titanium-50 pt-24 pb-20">
    ...
    <section class="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
      <div v-if="page.items.length" class="grid grid-cols-1 gap-x-14 gap-y-16 sm:grid-cols-2 lg:grid-cols-3">
        <article v-for="item in page.items" :key="`${item.title}-${item.imageUrl}`" class="group">
          <div class="flex min-h-[380px] items-center justify-center border border-titanium-200 bg-white p-4 shadow-sm shadow-titanium-200/60 transition-all duration-300 group-hover:-translate-y-1 group-hover:shadow-xl group-hover:shadow-titanium-200/80">
            <img
              :src="item.imageUrl"
          ...
          <div v-if="item.title || item.desc" class="mt-4 text-center">
            <h2 v-if="item.title" class="text-sm font-semibold text-titanium-950">{{ item.title }}</h2>
            <p v-if="item.desc" class="mt-1 text-xs leading-relaxed text-titanium-500">{{ item.desc }}</p>
          </div>
        </article>
      </div>
      ...
      <div v-else class="mx-auto max-w-2xl rounded-xl border border-dashed border-titanium-300 bg-white p-10 text-center">
        ...
        <p class="mt-2 text-sm leading-relaxed text-titanium-500">
          The gallery is connected to the admin panel. Upload real certificate images in Site Settings to publish them here.
        </p>
      </div>
    </section>
  </div>
